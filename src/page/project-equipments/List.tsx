...
import { getProjectEquipments, deleteProjectEquipment } from '../../services/projectEquipmentService';
import type { ProjectEquipment } from '../../types/project-equipment';
...
export const EquipmentAllocationList: React.FC = () => {
  const [allocations, setAllocations] = useState<ProjectEquipment[]>([]);
  ...
  const loadAllocations = async () => {
    try {
      setLoading(true);
      const response = await getProjectEquipments();
  ...
  const columns: Column<ProjectEquipment>[] = [
    { 
      header: 'Projeto', 
      accessor: (row) => row.project ? row.project.title : '--'
    },
    { 
      header: 'Equipamento', 
    ...
    { 
      ...
      accessor: (row) => (
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button onClick={() => navigate(`/projects-equipments/${row.id}/edit`)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', transition: 'color 0.2s' }} title="Editar Alocação"><Edit2 size={18} /></button>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button onClick={() => navigate('/projects-equipments/create')}>
